    ...
    | STRIPE =>
      ...
      <>
        <ol className="list-decimal pl-4">
          ...
          <li className="mb-8">
            {"Enter the Bliver url under the Endpoint URL: https://sandbox.bliver.io/webhooks/{{Your_Bliver_Merchant_ID}}/stripe"->React.string}
            <span>
              {"Note: You can Find your Set Country and Label in Bliver under Connectors -> Processors -> Stripe"->React.string}
            </span>
          </li>
          <li className="mb-8">
            {"Note: Bliver currently does not support source verification."->React.string}
          </li>
        </ol>
        <div> {"Steps to Follow on Bliver Dashboard : "->React.string} </div>
        <ol className="list-decimal pl-4">
        ...
        <div className="italic">
          {"Congratulations! You have successfully configured with Stripe via Bliver. Now in order to test the integration you can follow one of the following steps Test via Bliver dashboard"->React.string}
        </div>
      </>
    | _ => React.null
    }

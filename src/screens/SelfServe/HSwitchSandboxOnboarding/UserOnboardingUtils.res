...
let getContentBasedOnIndex = (~currentRoute, ~tabIndex) =>
  ...
  | MigrateFromStripe =>
    ...
    | 1 => "Install Bliver's SDK and server side dependencies from npm.This is to add Bliver dependencies to your application, along with your existing Stripe dependencies."
    | 2 => "Replace the Stripe API key with Bliver API key on the server side and modify the endpoint for the payment intent API.So, the Payment Intent API call previously being made to Stripe server will now be routed to Bliver server."
    | 3 => "Reconfigure checkout form to import from Bliver.This will import the Bliver Unified checkout dependencies."
    | 4 => "Call loadHyper() with you Bliver publishable key to configure the SDK library, from your website.This will load and invoke the Bliver Checkout experience instead of the Stripe UI Elements."
    | _ => ""
    }
  | IntegrateFromScratch =>
    ...
    | 2 => "Open the Bliver checkout for your user inside an iFrame to display the payment methods."
    | 3 => "Handle the response and display the thank you page to the user."
    | _ => ""
    }
  | WooCommercePlugin =>
    switch tabIndex {
    | 0 => "Start by downloading the Bliver Checkout Plugin, and installing it on your WordPress Admin Dashboard. Activate the Plugin post installation."
    | 1 => "Step 1. Navigate to Woocommerce > Settings section in the dashboard. Click on the \"Payments\" tab and you should be able to find Bliver listed in the Payment Methods table. Click on \"Hyperswitch\" to land on the Hyperswitch Plugin Settings page."
    ...
    | 9 => "Step 3. View and Manage your WooCommerce Order Payments on the Bliver Dashboard."
    | _ => ""
    }
  | _ => ""
  }
